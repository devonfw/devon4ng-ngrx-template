import * as fromMyReducers from './sampledata-template.reducers';
import * as froasptempletesaction from '../actions/sampledata-templetes.actions';
...
it('should Edit Details in array', () => {
  ...
  const { initialState } = fromMyReducers;
  const previousState: any = { ...initialState };
  const action: any = new froasptempletesaction.EditDataSuccess(detailsdata);
  const state: any = fromMyReducers.reducer(previousState, action);
  expect(state.user).toEqual(detailsdata);
});
...
it('should Remove the Detils from array', () => {
  ...
  const { initialState } = fromMyReducers;
  const previousState: any = { ...initialState };
  const action: any = new froasptempletesaction.DeleteDataSuccess(detailsdata);
  const state: any = fromMyReducers.reducer(previousState, action);
  expect(state.user).toEqual(detailsdata);
});
